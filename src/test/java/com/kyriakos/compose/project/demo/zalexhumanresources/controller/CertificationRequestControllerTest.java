...
public class CertificationRequestControllerTest {
    ...
    void getEmployeeCertifications_success()  {
        ...
        when(certificationRequestService.getEmployeeCertifications(any(), anyInt(), anyInt(), any(), any()))
                .thenReturn(mockPage);
        ...
        assertThat(mockMvc.get().uri("/certification-requests")
                ...
                .hasStatusOk()
                .bodyJson()
                .hasPathSatisfying("$.content[0].address_to", v -> assertThat(v).asString().isEqualTo(HR_DEPARTMENT))
                .hasPathSatisfying("$.content[0].status", v -> assertThat(v).asString().isEqualTo("OPEN"))
